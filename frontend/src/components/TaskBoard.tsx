...
import {
  apiListTasks, apiUpdateStatus, apiCreateTask, computePriority, progressSummary, apiAssignSprint
} from '../lib/tasks';
...
export default function TaskBoard() {
  ...
  const onAction = async (id: string, action: 'start' | 'block' | 'unblock' | 'finish') => {
    try {
  ...
  const onQuickAdd = async () => {
    try {
      ...
      const points = 3;
      const t: Omit<Task, 'id'> = { title, points, status: 'todo', due: null, labels: [] };
  ...
  const Column = ({ title, items }: { title: string; items: Task[] }) => (
    <div style={{ flex: 1, minWidth: 240, padding: 12, border: '1px solid #eee', borderRadius: 12 }}>
      <h3 style={{ marginTop: 0 }}>{title} ({items.length})</h3>
      <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'grid', gap: 8 }}>
        {items.map(t => (
          <li key={t.id} style={{ padding: 12, border: '1px solid #ddd', borderRadius: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 6 }}>
              {(t.labels ?? []).map(l => (
                <span key={l} style={{ border: '1px solid #ccc', borderRadius: 6, padding: '2px 6px', fontSize: 12 }}>#{l}</span>
              ))}
            </div>
    ...
    <div style={{ marginTop: 24 }}>
      <div style={{ display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div>
          ...
          <input type="number" value={capacity} onChange={e => setCapacity(parseInt(e.target.value || '0', 10))} style={{ width: 80 }} />
          <button onClick={onAssign} style={{ marginLeft: 8 }}>assign sprint</button>
        </div>
        <div style={{ marginLeft: 'auto' }}>
          <strong>Progress:</strong> {prog.done}/{prog.total} done • {prog.completionPct}% • overdue: {prog.overdue}
        </div>
      </div>
